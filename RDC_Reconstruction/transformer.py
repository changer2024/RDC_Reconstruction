...
class VQGANTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        start_indices = new_indices[:, :0]
        indices_gen = self.sample(start_indices, sos_tokens, steps=indices.shape[1])
        x_rec = self.z_to_image(indices_gen)

        return indices_gen, indices, x_rec
